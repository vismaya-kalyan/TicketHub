...
public class SoundSystemList extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        pw.print("<a style='font-size: 24px;'>" + name + " Sound System's</a>");
        ...
        for (Map.Entry<String, SoundSystem> entry : hm.entrySet()) {
            ...
            if (soundsystem.getDiscount() > 0) {
                pw.print("<h3>Discount: " + soundsystem.getDiscount() + "</h3>");
            }
            pw.print("<li id='item'><img src='images/sounds/" + soundsystem.getImage() + "' alt='' /></li>");

            pw.print("<li><form method='post' action='Cart'>" + "<input type='hidden' name='name' value='"
                    + entry.getKey() + "'>" + "<input type='hidden' name='type' value='soundsystems'>"
